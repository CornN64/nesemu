...
static void system_video(bool draw)
{
   ...
   //   vid_blit(nes.vidbuf, 0, (NES_SCREEN_HEIGHT - NES_HEIGHT) / 2,
   //            0, 0, NES_SCREEN_WIDTH, NES_HEIGHT);
...
void nes_emulate(void)
{
   ...
   while (false == nes.poweroff)
   {
      ...
      else if (frames_to_render > 1)
      {
         ...
         system_video(false);
		 //printf("%d\n", nofrendo_ticks);	//Show missed frames
      }
      else if ((1 == frames_to_render && true == nes.autoframeskip) || false == nes.autoframeskip)
      {
		 frames_to_render = 0;
#ifdef SHOW_RENDER_VIDEO_TIME
		 uint xF = micros();	//start time
         nes_renderframe(true);
		 uint xN = micros();	//render time
         system_video(true);
		 uint xE = micros();	//SPI time
		 printf("%d\t%d\n", xN - xF, xE - xN);	//show render and video transafer time in micro sec.
#else
         nes_renderframe(true);
         system_video(true);
#endif
      ...
      if (nes.saveSramCountdown > 0)
      {
         ...
         if (nes.saveSramCountdown == 0 && xSramSaveHandle == NULL)
         {
            ...
            xTaskCreatePinnedToCore(
                ...
                1);					// Core where the task should run 0,1,tskNO_AFFINITY
         }
      }
   }
}
