...
{
   uint8 ines_magic[4]    ;	// iNES magic number
   uint8 rom_banks        ;	// number of PRG-ROM banks (16KB each)
   uint8 vrom_banks       ;	// number of CHR-ROM banks (8KB each)
   uint8 rom_type         ;	// control bits
   uint8 mapper_hinybble  ;	// control bits
   uint8 reserved[8]      ;	// PRG-RAM size (x 8KB) + 7 Bytes padding
} inesheader_t;
...
#define READ_BUFFER_SIZE 4096	//do not change this, its the EEPROM block size on ESP32
...
char *osd_getromdata()
{
  ...
  if (part == 0)
  ...
  else
	{
		...
		printf("partition subtype = %d.\n", part->subtype);
		printf("partition starting address = %x.\n", part->address);
		printf("partition size = %x.\n", part->size);
  ...
  // Copy the file contents into EEPROM memory if not already copied (verify content before writing to avoid erase cycles)
  char *buffer = malloc(READ_BUFFER_SIZE);
  char *buffer_cmp = malloc(READ_BUFFER_SIZE);
  int offset = 0;
  while (1)
  {
	int numbytes = fread(buffer, 1, READ_BUFFER_SIZE, rom);
	if (numbytes == 0)
		break;
	
    esp_partition_read(part, offset, buffer_cmp, READ_BUFFER_SIZE);
	
	for(int i=0; i<numbytes; i++)
	{
		if (buffer[i] != buffer_cmp[i])
		{	
			printf("Rewrite @ offset %X\n", offset);
			esp_partition_erase_range(part, offset, READ_BUFFER_SIZE);
  ...
  free(buffer);
  free(buffer_cmp);
  printf("Loaded/Verified %d ROM bytes mapped on EEPROM area\n", fileSize);
#else
