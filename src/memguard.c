...
static void mem_deleteblock(void *data, char *file, int line)
{
   ...
   for (i = 0; i < MAX_BLOCKS; i++)
   {
      if (data == mem_record[i].block_addr)
      {
         ...
         {
            printf("mem_deleteblock 0x%08X at line %d of %s -- block corrupt",
   ...
   printf("mem_deleteblock 0x%08X at line %d of %s -- block not found",
           (uint32) data, line, file);
}
#endif /* MEMORY_DEBUG */
...
void *_my_malloc(int size, char *file, int line)
{
   ...
   if (NULL == temp)
   {
      sprintf("malloc: out of memory at line %d of %s.  block size: %d\n",
              line, file, size);
   }
...
void _my_free(void **data, char *file, int line)
{
   ...
   {
      printf("free: attempted to free NULL pointer at line %d of %s\n",
              line, file);
   }
...
void *_my_malloc(int size)
{
   ...
   if (NULL == temp)
   {
      printf("malloc: out of memory.  block size: %d\n", size);
   }
...
void mem_checkleaks(void)
{
   ...
   if (mem_blockcount)
   {
      printf("memory leak - %d unfreed block%s\n\n", mem_blockcount, 
         mem_blockcount == 1 ? "" : "s");
      ...
      for (i = 0; i < MAX_BLOCKS; i++)
      {
         ...
         {
            printf("addr: 0x%08X, size: %d, line %d of %s%s\n",
...
void mem_checkblocks(void)
{
   ...
   for (i = 0; i < MAX_BLOCKS; i++)
   {
      if (mem_record[i].block_addr)
      {
         if (mem_checkguardblock(mem_record[i].block_addr, GUARD_LENGTH))
         {
            printf("addr: 0x%08X, size: %d, line %d of %s -- block corrupt\n",
                    (uint32) mem_record[i].block_addr,
                    mem_record[i].block_size,
                    mem_record[i].line_num,
                    mem_record[i].file_name);
